<template>
  <div class="loginPage">
    <div class="loginPage-header">
      <div class="loginPage-back" @click="$router.push('/home')"></div>
      <div class="loginPage-title">京东登录注册</div>
    </div>
    <div class="loginPage-main">
      <div class="loginPage-mian-Box">
        <input
          @keyup.enter="mysubmit"
          @focus="getFocus(1)"
          @blur="loseFocus(1)"
          @input="fun"
          v-model="username"
          placeholder="用户名/邮箱/手机号"
          class="loginPage-input"
          type="text"
        />
        <img
          @click="deletedataa"
          class="loginPage-mian-Box-delete"
          v-if="focusa"
          src="data:image/png;base64,"
        />
      </div>
      <div class="loginPage-mian-Box">
        <input
          @keyup.enter="mysubmit"
          @focus="getFocus(2)"
          @blur="loseFocus(2)"
          @input="fun"
          v-model="password"
          placeholder="请输入密码"
          :type="showtype"
          class="loginPage-input"
        />
        <div class="loginPage-mian-Box-other">
          <div @click="changeShowtype">
            <img v-if="showIcon" class="loginPage-mian-Box-other-img" src="data:image/png;base64,">
            <img v-else class="loginPage-mian-Box-other-img" src="data:image/png;base64,">
          </div>
          <div style="transform:translateY(-1px);font-size:16px;margin:0 3px;line-height:40px">|</div>
          <div style="font-size:14px;line-height:40px">忘记密码</div>
        </div>
        <img
          style="right:120px"
          @click="deletedatab"
          class="loginPage-mian-Box-delete"
          v-if="focusb"
          src="data:image/png;base64,"
        />
      </div>
      <div class="loginPage-mian-Box">
        <button @click="mysubmit" :class="btnbgc" class="loginBtn">登录</button>
      </div>
      <div class="loginPage-mian-Box">
        <div class="otherLogin">短信验证码登录</div>
        <div class="otherLogin">手机快速注册</div>
      </div>
    </div>
    <div class="loginPage-other">
      <div class="txt-otherLogin">其他登录方式</div>
      <div class="loginPage-otherQW">
        <div class="loginPage-otherQW-item">
          <img
            src="data:image/png;base64,"
          />
          QQ
        </div>
        <div class="loginPage-otherQW-item">
          <img
            src="data:image/png;base64,"
          />
          微信
        </div>
      </div>
      <div class="loginPage-otherZ">
        登录即代表您已同意
        <a
          href="https://in.m.jd.com/help/app/private_policy.html"
          style="color: rgb(101, 158, 224);"
        >京东隐私政策</a>
      </div>
    </div>
  </div>
</template>

<script>
import Tabbar from "../components/public/tabBar.vue";
const components = {
  Tabbar
};
// 登录
export default {
  name: "login",
  created () {
    if(this.$store.state.loginModule.islogin){
      this.$router.push("/mine")
    }
  },
  data() {
    return {
      username: "",
      password: "",
      btnbgc: "unlogin",
      focusa: false,
      focusb: false,
      showIcon: true,
      showtype: "password"
    };
  },
  methods: {
    mysubmit(){
      if(this.username.trim()==""||this.password.trim()==""){
        alert("登录信息有误")
        return
      } else {
        this.$store.commit("loginFun",{
        username: this.username.trim(),
        password: this.password.trim()
        })
        this.username = "";
        this.password = "";
        let that = this
        setTimeout(()=>{
          if( that.$store.state.loginModule.islogin ){
            that.$router.push("/mine")
          }else{
            alert("无效的信息")
          }
        },200)
      }
    },
    changeShowtype(){
      if(this.showIcon){
        this.showtype = "text"
      }else{
        this.showtype = "password"
      }
      this.showIcon = !this.showIcon
    },
    deletedataa() {
      console.log("删除");
      this.username = "";
      this.btnbgc = "unlogin";
    },
    deletedatab() {
      console.log("删除");
      this.password = "";
      this.btnbgc = "unlogin";
    },
    getFocus(n) {
      if (n == 1) {
        this.focusa = true;
      } else {
        this.focusb = true;
      }
    },
    loseFocus(n) {
      let that = this;
      setTimeout(() => {
        if (n == 1) {
          that.focusa = false;
        } else {
          that.focusb = false;
        }
      }, 0);
    },
    fun() {
      if (this.username.trim() !== "" && this.password.trim() !== "") {
        this.btnbgc = "haslogin";
      } else {
        this.btnbgc = "unlogin";
      }
    }
  },
  components
};
</script>

<style scoped>
.loginPage {
  width: 100%;
  height: 100%;
}
.loginPage-header {
  position: relative;
  height: 42px;
  background-color: #fff;
}
.loginPage-title {
  position: absolute;
  left: 50%;
  top: 50%;
  transform: translate(-50%, -50%);
}
.loginPage-back {
  top: 10px;
  left: 15px;
  background-image: url(data:image/png;base64,);
  width: 24px;
  height: 24px;
  overflow: hidden;
  display: block;
  position: absolute;
  background-repeat: no-repeat;
  background-size: 100% auto;
  background-position: 0;
}
.loginPage-main {
  display: flex;
  margin: 10px auto 10px auto;
  flex-direction: column;
  width: 85%;
}
.loginPage-other {
  position: relative;
}
.txt-otherLogin {
  position: relative;
  color: rgba(0, 0, 0, 0.2);
  margin: 0 auto;
  width: 85%;
  background-color: #ccc;
  font-size: 13px;
  background: #fff;
  text-align: center;
}
.txt-otherLogin::before {
  position: absolute;
  content: "";
  display: block;
  left: 0;
  width: 30%;
  top: 50%;
  transform: translateY(-50%);
  height: 1px;
  background-color: rgba(0, 0, 0, 0.2);
}
.txt-otherLogin::after {
  position: absolute;
  right: 0;
  content: "";
  display: block;
  width: 30%;
  top: 50%;
  transform: translateY(-50%);
  height: 1px;
  background-color: rgba(0, 0, 0, 0.2);
}
.loginPage-otherQW {
  margin-top: 10px;
  display: flex;
  justify-content: center;
}
.loginPage-otherQW-item {
  margin: 20px;
  width: 50px;
  height: 60px;
  font-size: 14px;
  text-align: center;
  color: #ccc;
}
.loginPage-otherQW-item > img {
  width: 100%;
}
.loginPage-otherZ {
  color: #ccc;
  font-size: 14px;
  text-align: center;
  margin-top: 10px;
  letter-spacing: 1px;
}
.loginPage-mian-Box {
  width: 100%;
  padding: 5px 0;
  display: flex;
  position: relative;
  justify-content: space-between;
}
.loginBtn {
  appearance: none;
  outline: none;
  border: none;
  color: #fff;
  width: 100%;
  height: 50px;
  border-radius: 50px;
  font-size: 17px;
}
.otherLogin {
  font-size: 14px;
  color: #ccc;
}
.loginPage-input {
  appearance: none;
  outline: none;
  border: none;
  width: 100%;
  border-bottom: 1px solid #ccc;
  padding: 15px 0;
  font-size: 16px;
  color: rgb(107, 107, 107);
}
.loginPage-input::-webkit-input-placeholder {
  color: rgb(163, 163, 163);
}
.unlogin {
  background-color: rgb(252, 182, 179);
}
.haslogin {
  background: linear-gradient(90deg, #f10000, #ff2000 73%, #ff4f18);
}
.loginPage-mian-Box-delete {
  width: 25px;
  height: 25px;
  position: absolute;
  top: 50%;
  transform: translateY(-47%);
  right: 0px;
  
}
.loginPage-mian-Box-other{
  position: absolute;
  right: 0;
  top: 50%;
  transform: translateY(-50%);
  width: 110px;
  height: 40px;
  color: rgb(150, 150, 150);
  display: flex;
  align-items: center;  
  justify-content: space-between;
}
.loginPage-mian-Box-other-img{
  width: 24px;
  transform: translateY(3px);
}
</style>
